...
use NFQ\SyliusOmnisendPlugin\Client\Response\Model\BatchItemsSuccess;
use NFQ\SyliusOmnisendPlugin\Client\Response\Model\BatchSuccess;
use NFQ\SyliusOmnisendPlugin\Client\Response\Model\CartDataSuccess;
...
class OmnisendClient implements LoggerAwareInterface, OmnisendClientInterface
{
    ...
    public function getCart(?string $cartId, ?string $channelCode): ?object
    {
        ...
        return $this->parseResponse($response, CartDataSuccess::class);
    }
    ...
    public function getBatchItems(string $batchId, ?string $channelCode): ?object
    {
        $response = $this->sendRequest(
            $this->messageFactory->create(
                'GET',
                self::API_VERSION . self::URL_PATH_BATCHES . '/' . $batchId . '/items',
            ),
            $channelCode,
        );

        return $this->parseResponse($response, BatchItemsSuccess::class);
    }
